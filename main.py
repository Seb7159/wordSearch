class WordSearch(object):
    ...
    def searchWordInGrid(self, word, i, j, direction, actual):
    ...
    def is_present(self, word):
        ...
        for i in range(self.ROW_LENGTH):
            for j in range(self.ROW_LENGTH):
                if self.searchWordInGrid(word, i, j, 0, word) is True or self.searchWordInGrid(word, i, j, 1, word) is True:
                    return True
        return False


# Create method to generate random grid
...
words_to_find = ['AS', 'YES', 'PEXIP', 'IS', 'SO', 'COOL']
